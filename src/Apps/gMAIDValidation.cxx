...
void MakePlots (void)
{
  ...
  for ( int ires = 0 ; ires < kNRes ; ++ires ) { 
    ...
    for( unsigned int i = 0 ; i < gQ2_binning_XSec + 1 ; ++i ) { 
      ...
      // If it is the delta, remove TMath::Sqrt(115./130.) factor:
      if( ires ==  kP33_1232 ) {
	A12_p[i] /= TMath::Sqrt(115./130.);
	A12_n[i] /= TMath::Sqrt(115./130.);
	A32_p[i] /= TMath::Sqrt(115./130.);
	A32_n[i] /= TMath::Sqrt(115./130.);
